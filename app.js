...
let recurTypeActif = 'quotidien'; // type de récurrence en cours de saisie

const JOURS_NOM = ['Dim','Lun','Mar','Mer','Jeu','Ven','Sam'];

// Appliquer thème immédiatement (avant DOMContentLoaded)
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.jour-check').forEach(label => {
  ...
  // Mettre à jour le bouton permission selon l'état actuel
  majBtnNotifPerm();
});

function initRecurUI() {
  // Activer "Quotidien" par défaut
  setRecurType('quotidien', document.querySelector('.recur-type-btn[data-type="quotidien"]'));
}
...
function setRecurType(type, btn) {
  recurTypeActif = type;
  document.querySelectorAll('.recur-type-btn').forEach(b => b.classList.remove('actif'));
  if (btn) btn.classList.add('actif');

  const joursBloc      = document.getElementById('recur-jours-bloc');
  const intervalleBloc = document.getElementById('recur-intervalle-bloc');

  // Afficher les jours pour hebdo et personnalisé
  joursBloc.style.display      = (type === 'hebdo' || type === 'personnalise') ? 'block' : 'none';
  // Afficher l'intervalle uniquement pour personnalisé
  intervalleBloc.style.display = type === 'personnalise' ? 'block' : 'none';
}

function getJoursCoches() {
  return Array.from(document.querySelectorAll('.jour-check.selected'))
...
function demanderPermissionNotifs() {
  ...
  if (Notification.permission === 'default') {
    Notification.requestPermission().then(p => {
      if (p === 'granted') { showToast('🔔 Notifications activées !'); majBtnNotifPerm(); }
    });
  }
}
...
function verifierRappels() {
  ...
  taches.filter(t => !t.faite && t.echeance === todayStr && t.heure === hhmm)
        .forEach(t => envoyerNotif('⏰ Rappel', t.texte));
  majBellBadge();
}
...
function ajouterTache() {
  ...
  const joursCoches = getJoursCoches(); // [0-6] selon JS (0=dim)
  ...
  const t = {
    ...
    // Pour hebdo/personnalisé : jours cochés ; pour personnalisé : intervalle en jours
  ...
  let extra = '';
  if (heure) extra += ` · rappel ${heure}`;
  if (dureeMinutes) extra += ` · ⏱ ${dureeMinutes}min`;
  if (isRecur && t.recurJoursSemaine?.length) extra += ` · 🔁 ${t.recurJoursSemaine.map(j=>JOURS_NOM[j]).join(',')}`;
  showToast('✅ Tâche ajoutée' + extra);
  ...
  document.querySelectorAll('.jour-check').forEach(l => l.classList.remove('selected'));
  setRecurType('quotidien', document.querySelector('.recur-type-btn[data-type="quotidien"]'));
  majCategoriesSelect();
  input.focus();
}
...
function afficher(filtre = 'toutes') {
  ...
  if (!visibles.length) { liste.innerHTML = '<div class="empty">Aucune tâche ici 🎉</div>'; majCompteur(); return; }
...
function labelRecur(t) {
  if (!t.recurType) return '';
  if (t.recurType === 'quotidien') return 'Quotidien';
  if (t.recurType === 'mensuel')   return 'Mensuel';
  if (t.recurType === 'hebdo' || t.recurType === 'personnalise') {
    if (t.recurJoursSemaine?.length) return t.recurJoursSemaine.map(j=>JOURS_NOM[j]).join('/');
    if (t.recurType === 'personnalise' && t.recurJours) return `/${t.recurJours}j`;
  }
  return t.recurType;
}
...
function renderSousTaches(t) {
  ...
  (t.sousTaches||[]).forEach((s,i) => {
    ...
    bd.onclick=()=>{t.sousTaches.splice(i,1); sauvegarder(); renderSousTaches(t);};
...
function tickTimer() {
  ...
  if(restant<=0){
    ...
    envoyerNotif('⏱ Session terminée !', taches.find(x=>x.id===timerData.tacheId)?.texte||'');
...
function demarrerChrono(tacheId,nom) {
  ...
  playSound('add'); showToast('⏱ Chrono : '+nom);
  afficher(filtreActif());
}
function tickChrono() {
...
// POMODORO
// ============================================================
const CIRCUMFERENCE=2*Math.PI*88; // r=88, dasharray=553

let pomoState={
  running:false, counting:false, paused:false,
  mode:'work', sessionsDone:0,
  timeLeft:25*60, totalTime:25*60,
  interval:null, countInterval:null,
};
...
function pomoDemarrer() {
  ...
  if(pomoState.paused) {
    ...
    showToast('▶ Session reprise !'); return;
  }
  ...
  // Compte à rebours 3→2→1→GO visible
  let countdown=3;
  ...
  el.textContent='3'; lbl.textContent='Préparez-vous !';
  circle.style.stroke='var(--border2)'; circle.style.strokeDashoffset=String(CIRCUMFERENCE);
  playSound('beep');

  pomoState.countInterval=setInterval(()=>{
    countdown--;
    if(countdown>0) { el.textContent=String(countdown); playSound('beep'); }
    else {
      ...
      document.getElementById('pomo-pause-btn').style.display='inline-flex';
      pomoRenderDisplay(); playSound('notif'); showToast('🍅 C\'est parti !');
      pomoState.interval=setInterval(pomoTick,1000);
    }
  },1000);
}
...
function pomoStop() {
  ...
  pomoState.interval=null; pomoState.countInterval=null;
  pomoState.running=false; pomoState.counting=false; pomoState.paused=false;
  pomoState.mode='work';
...
function pomoSessionFinie() {
  ...
  if(pomoState.mode==='work') {
    ...
    envoyerNotif('🍅 Pomodoro terminé !', tacheNom?`Session : ${tacheNom}`:'Prenez une pause !');
    pomoState.mode=longue?'long':'short';
    pomoState.timeLeft=(longue?s.long:s.short)*60; pomoState.totalTime=pomoState.timeLeft;
    showToast(`🍅 Fini ! ${longue?'Grande pause 🌴':'Pause courte ☕'}`);
  } else {
    ...
    showToast('☕ Pause finie ! Au boulot 💪');
    pomoState.mode='work'; pomoState.timeLeft=s.work*60; pomoState.totalTime=s.work*60;
  }
  document.getElementById('pomo-start-btn').textContent  ='▶ Démarrer';
  document.getElementById('pomo-start-btn').style.display='inline-flex';
...
function renderPomoHistory() {
  ...
  if(!pomoHistory.length){c.innerHTML='<div style="color:var(--text-muted);font-size:13px;padding:8px 0">Aucune session encore.</div>'; return;}
  const icons={work:'🍅',short:'☕',long:'🌴'}, noms={work:'Session de travail',short:'Pause courte',long:'Pause longue'};
  c.innerHTML=pomoHistory.slice(0,10).map(h=>
...
function renderCategories() {
  ...
  if(!categories.length){c.innerHTML='<div class="empty">Aucune catégorie.</div>'; return;}
...
function genererRecurrentes() {
  ...
  taches.filter(t=>t.recurrente).forEach(t=>{
    ...
    if(t.recurType==='quotidien') {
      doGenerer=Math.floor((today-derniere)/86400000)>=1;
    } else if(t.recurType==='mensuel') {
      doGenerer=Math.floor((today-derniere)/86400000)>=30;
    } else if(t.recurType==='hebdo' || t.recurType==='personnalise') {
      // Si des jours de semaine sont spécifiés, générer si aujourd'hui est un jour coché et pas déjà fait aujourd'hui
      if(t.recurJoursSemaine?.length) {
        doGenerer = t.recurJoursSemaine.includes(todayDow) && t.derniereGen !== todayStr;
      } else if(t.recurType==='personnalise' && t.recurJours) {
        doGenerer=Math.floor((today-derniere)/86400000)>=t.recurJours;
      } else {
        doGenerer=Math.floor((today-derniere)/86400000)>=7;
      }
    }
...
function renderRecurrentes() {
  ...
  if(!recur.length){c.innerHTML='<div class="empty">Aucune tâche récurrente.<br>Cochez "Récurrente" à la création.</div>'; return;}
  c.innerHTML=recur.map(t=>{
    const cat=categories.find(c=>c.id===t.catId);
    const badge_cls=t.recurType==='personnalise'?'recur-perso-badge':'recur-badge';
    const badge_lbl=labelRecur(t);
    const freq=getFreqLabel(t);
    const joursTags=t.recurJoursSemaine?.length
      ? `<div class="recur-jours-tags">${t.recurJoursSemaine.map(j=>`<span class="recur-jour-tag">${JOURS_NOM[j]}</span>`).join('')}</div>`
      : '';
    return `<div class="recur-card">
      <div class="recur-icon">${cat?cat.icon:'🔁'}</div>
      <div class="recur-info">
        <div class="recur-name">${t.texte}</div>
        <div class="recur-freq">${freq}${cat?' · '+cat.nom:''}</div>
        ${joursTags}
      </div>
...
function getFreqLabel(t) {
  if(t.recurType==='quotidien') return 'Chaque jour';
  if(t.recurType==='mensuel')   return 'Chaque mois';
  if(t.recurType==='hebdo')     return t.recurJoursSemaine?.length ? 'Hebdo — jours sélectionnés' : 'Chaque semaine';
  if(t.recurType==='personnalise') {
    if(t.recurJoursSemaine?.length) return 'Jours personnalisés';
    if(t.recurJours) return `Tous les ${t.recurJours} jours`;
    return 'Personnalisé';
  }
  return '';
}
...
function afficherRappels() {
  ...
  c.innerHTML='';
  if(!avecRappel.length){c.innerHTML='<div class="empty">Aucun rappel défini.<br>Ajoutez une tâche avec date + heure.</div>'; return;}
